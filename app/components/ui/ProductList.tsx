	...
	const renderGridItem = (data: XtraderProductResp) => {
		...
		const renderStockStatus = () => {
			...
			if (_stockStatus === 'In Stock') {
				return (
					<span className={` ml-3 product-badge status-instock`}>
						{_stockStatus}
					</span>
				);
			} else {
				return (
					<span className={` ml-3 product-badge status-outofstock`}>
						{_stockStatus}
					</span>
				);
			}
				...
				<div className="product-grid-item card">
					<div className="product-grid-item-top">
						<div className="flex flex-row flex-wrap mb-3">
							...
							<span className="ml-3 product-category">{data.catName}</span>
							<div className="align-content-end">{renderStockStatus()}</div>
						</div>
			...
			<div className="col-12">
				<div className="product-list-item">
					<a
						...
						<div className={`${styles.name} font-medium`}>{data.name}</div>
