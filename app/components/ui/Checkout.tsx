...
export default function Checkout(
	...
	// console.log(`pathNameArray ${JSON.stringify(pathNameArray, null, 2)}`);
	const cart = useBasket();
	const [uiMode, setUiMode] = useState<string>(props.uiMode);
	...
	// 	console.log(`calc delivery charges called`);
	...
	// 			console.log(`charges filter called`);
	...
	const onDeliverySubmit = async (formData: DELIVERY_INFO_TYPE) => {
		...
		console.log(
			`formData.shipper ${JSON.stringify(formData.shipper, null, 2)}`
		);
		if (formData.shipper) {
		...
		console.log(`line 225 payable ${payable}`);
		const total = {
			id: 3,
			title: 'Total',
			amount: payable,
		...
		_lines.push(total);
		console.log(`_lines ${JSON.stringify(_lines, null, 2)}`);
		setLines(_lines);
		setActiveIndex(2);
	};
	...
	const selectedShipperTemplate = (
		...
		if (option) {
			console.log(`selectedShipperTemplate ${JSON.stringify(option.amount)}`);
			return (
	...
	const cartLineItems = () => {
		...
		console.log(`items[0].item ${JSON.stringify(items[0].item)}`);
			...
			return (
				<li key={i} className="flex flex-column max-w-full md:flex-row mt-4">
					<div className="flex lign-items-center justify-content-center">
						...
						{/* <div className="flex align-items-center justify-content-center"> */}
						<div className="flex align-items-center justify-content-center">
							<span className="font-semibold ">£ {p.linePrice}</span>
						</div>
						{/* </div> */}
					</div>
				</li>
	...
	const handleCountryChange = (e: { value: number }) => {
		console.log(`handleCountryChange called with ${e.value}`);

		const id: number = e.value;
		setCountryEntered(id);
		const selectedCountry = props.countries.find((c) => c.id === e.value);
		console.log(`country found ${JSON.stringify(selectedCountry, null, 2)}`);
		// if (selectedCountry) {
		...
		console.log(`charges ${JSON.stringify(props.charges, null, 2)}`);
		const _shippers: DELIVERY_CHARGE_TYPE[] = props.charges.filter(
			(c: DELIVERY_CHARGE_TYPE) => {
				const minWeight = c.minWeight;
				const maxWeight = c.maxWeight;
				console.log(
					`charge min weight ${typeof minWeight} maxWeight ${maxWeight}`
				);
				console.log(`zero test ${minWeight === 0.0}`);
				if (minWeight === 0 && maxWeight === 0) {
					console.log(`include ${c.courier?.name} as 0 weight`);
					return c;
				} else if (
					c.country?.id === id &&
					weight > minWeight &&
	...
	const handlePaymentSubmit: React.FormEventHandler<HTMLFormElement> = async (
		...
		e.preventDefault();
		setLoading(true);
		console.log('paynow handlePaymentSubmit called');

		const data = new FormData();

		const deliveryInfo: DELIVERY_INFO_TYPE | undefined = cart.deliveryInfo;
		// console.log(
		// 	`handlePaymentSubmit deliveryInfo ${JSON.stringify(
		// 		deliveryInfo,
		// 		null,
		// 		2
		// 	)}`
		...
		// console.log(
		// 	`handlePaymentSubmit called with items ${JSON.stringify(items, null, 2)}`
		// );
		if (items) {
			...
			items.map((i: basketItemType) => {
				...
				console.log(`num models is ${models.length}`);
				const _prod: ORDER_PRODUCT = {
					model: item.model,
					quantity: models.length + 1,
				};
				if (item.attributes) {
				}
			...
			// console.log(`eans ${JSON.stringify(eans, null, 2)}`);
			/** save order  */
			if (deliveryInfo) {
				...
				alert(`edcOrder ${JSON.stringify(customerOrder, null, 2)}`);
				console.log(`edcOrder ${JSON.stringify(customerOrder, null, 2)}`);
				if (1 === 1) {
					return;
				}
				try {
					...
					console.log(
						`called /api/xtrader/custOrder custOrder status: ${custOrderResp.status} text:${custOrderResp.statusText}`
					);
					if (!custOrderResp.ok) {
						console.error(
					...
					console.log(
						`returned cust order ${JSON.stringify(
							JSON.stringify(custOrder, null, 2)
						)}`
					);
					//TODO: revert to order response from api route
					// const custOrder: orderResponse = {
					// 	status: '200',
					// 	orderMessage: { orderNumber: 123, orderId: 'orderId' },
					// };
					setOrderId(custOrder.orderMessage.orderId);
					setOrderNumber(custOrder.orderMessage.orderNumber);

					console.log(
						`custOrder from route hander ${JSON.stringify(custOrder, null, 2)}`
					);
					...
					console.log(`data for stripe ${JSON.stringify(data, null, 2)}`);
					...
					console.log(`data before call to stripe ${JSON.stringify(data)} `);
	...
	const shipperDropdownFocus = (e: FocusEvent<HTMLInputElement>) => {
		...
		const _shippers = shippers.map((shipper) => {
			...
			console.log(
				`name ${JSON.stringify(
					shipper.courier?.name,
					null,
					2
				)} amount ${JSON.stringify(
					shipper.amount
				)} deliveryAmount ${JSON.stringify(shipper.deliveryCharge)}`
			);
	...
	const isShippersDisabled = (): boolean => {
		console.log(
			`shippers.length ${shippers.length} shipPostCode.length ${shipPostCode.length}`
		);
	...
	return (
		<>
			{/* {console.log(`pathNmase is ${pathname}`)} */}
